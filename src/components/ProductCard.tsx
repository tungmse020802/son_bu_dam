...
export function ProductCard({ product, onAdd }: ProductCardProps) {
  ...
  return (
    <article className={`product-card product-card-premium ${isSoldOut ? 'product-card-soldout' : ''}`}>
      <div className="product-image-wrap">
        <img src={product.image} alt={product.name} className="product-image" />
        <span className="badge">{product.grade}</span>
        <div className="product-floating-chip">
          <Star size={14} />
          {product.arEnabled ? 'AR sưu tầm' : 'Thẻ sưu tầm'}
        </div>
      </div>
      <div className="product-content product-content-refined">
        ...
        <div className="product-card-actions product-card-actions-stacked">
          <button className="primary-btn full" onClick={() => onAdd(product.id)} disabled={isSoldOut}>
            <Boxes size={16} />
            {isSoldOut ? 'Tạm hết hàng' : 'Thêm vào giỏ hàng'}
          </button>
          <span className={`product-note ${isLowStock ? 'warning' : ''}`}>
            {isLowStock ? <AlertTriangle size={14} /> : <Sparkles size={14} />}
            {isLowStock ? 'Ưu tiên chốt đơn sớm để giữ combo' : 'Gợi ý học kèm quiz'}
          </span>
        </div>
      </div>
    </article>
